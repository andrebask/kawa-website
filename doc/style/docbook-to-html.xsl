...
<script language="JavaScript" type="text/javascript" src="style/utils.js"> </script>
</xsl:template>
...
-->
    <xsl:if test="phrase[@role='category']">
      <span class="kind"><xsl:value-of select="phrase[@role='category']/emphasis"/></span><span class="ignore">: </span>
    </xsl:if>
    <xsl:apply-templates/>
  </p>
</xsl:template>
...
<xsl:template name="section.toc">
  ...
  <div class="toc">
    <ul>
      <xsl:apply-templates select="//chapter" mode="chunk-toc">
...
<xsl:template match="*" mode="chunk-toc">
  ...
  <xsl:if test="$depth=1 or ($depth&lt;=$context-depth and @label=$toc-context/ancestor-or-self::*/@label) or ($depth = $context-depth and (parent::*/@label = $toc-context/parent::*/@label)) or parent::*/@label = $toc-context/@label">
  <li>
